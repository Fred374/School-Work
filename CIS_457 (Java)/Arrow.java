...
class CvArrow extends Canvas {
   ...
   public void paint(Graphics g) {
      ...
      // Show initial arrow:
      drawArrow(g, x, y);
	  System.out.println("Give rotation center: ");
	  Scanner scan=new Scanner(System.in);
	  int xc=scan.nextInt();
	  int yc=scan.nextInt();
	  System.out.println("Give rotation magnitude(degrees): ");
	  int a=scan.nextInt();
	  float phi = (float) ((a*Math.PI/180)%(2*Math.PI)),
			c = (float) Math.cos(phi), s = (float) Math.sin(phi), 
            r11 = c, r12 = s, r21 = -s, r22 = c;
      for (int j = 0; j < 4; j++) {
         float xNew = x[j] * r11 +xc + y[j] * r21, 
               yNew = x[j] * r12 + y[j] * r22 + yc;
         x[j] = xNew; y[j] = yNew;
      }
